...
- **🎯 分角度分析** - 每个维度4-6个子角度深入剖析，逻辑清晰
- **💼 可操作性强** - 给出具体的配置建议和应对策略
- **⚡ LCEL语法** - 使用LangChain Expression Language，代码简洁优雅
- **📊 全数据整合** - AI分析整合所有数据源：7×24快讯、市场指数、金价、成交量、分时数据、板块、基金持仓
- **📄 双输出模式** - 生成markdown报告文件 + 控制台智能显示（自动过滤markdown语法）
...
- **系统角色**：资深金融分析师
- **输出长度**：300-400字
- **数据输入**：7×24快讯、市场指数、金价走势、成交量、分时数据、领涨板块
- **分析角度**：
  - 结合7×24快讯，分析当前市场热点和重要事件
  - 主要指数走势特征和相互关系
  - 市场所处阶段判断（上涨/震荡/调整）
  - 市场情绪和资金流向特征（结合成交量和分时数据）
  - 国内外市场对比及关键影响因素
  - 金价走势对市场的影响
...
- **系统角色**：基金投资顾问
- **输出长度**：300-400字
- **数据输入**：基金持仓、市场指数、成交量、板块表现
- **分析角度**：
  - 持仓基金表现和风险特征评估
  - 持仓与市场环境匹配度分析（结合成交量和板块轮动）
  - 具体调仓建议（增持/减持/持有）
  - 优异基金背后原因及可持续性
  - 仓位配置和风险敞口优化方向
...
- **系统角色**：风险管理专家
- **输出长度**：250-350字
- **数据输入**：市场指数、金价、成交量、分时数据、板块表现、基金持仓
- **分析角度**：
  - 当前市场主要风险点识别（结合成交量萎缩/放大、分时走势等）
  - 可能引发调整的触发因素
  - 持仓基金风险暴露评估
  - 风险防控建议和应对策略
  - 需要关注的风险信号（包括技术面和资金面）
...
#### 2. Markdown文件（完整格式）
- 保存位置：`reports/AI市场深度分析报告_YYYYMMDD_HHMMSS.md`
- 文件结构：
  ```markdown
  # AI市场深度分析报告
